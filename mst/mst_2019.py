...
def approx_mst(n, eps, max_w):
    ...
    if max_w > 1:
        ...
        c_bars = approx_cc_simple(n, max_w, eps, max_w, d_bar, time_limit=0.55, start_time=time.time(), read_new_verts=True)
    else:
        c_bars = approx_cc_simple(n, max_w, eps/300, max_w, d_bar, time_limit=0.55, start_time=time.time(), read_new_verts=True)
