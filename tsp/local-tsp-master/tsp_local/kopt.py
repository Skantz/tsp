...
from tsp_local.logger import Log
...
class KOpt(TSP):
        ...
        for t2i in around:
            ...
            if xi not in joined and xi not in broken:
                ...
                if str(new_tour) in self.solutions:
                    Log.debug("Skip already found tour.")
                    return False

                Log.debug("Choose x: {} = {}".format(xi, TSP.dist(last, t2i)))
                ...
                if is_tour and relink > 0:
                    ...
                    Log.info("Found {}-opt with gain {}".format(
                        len(added), relink))
                    Log.debug("X: {} - Y: {}".format(removed, added))
    ...
    def chooseY(self, tour, t1, t2i, gain, broken, joined):
        ...
        for node, (_, Gi) in ordered:
            ...
            Log.debug("Choose y: {} = {}".format(yi, TSP.dist(t2i, node)))
